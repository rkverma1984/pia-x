...
def convert_Residue2TMs():
    ...
    for i in range(137-5+2, 179+2): #TM3
        OPRM_BW_dic[i]='TM3.'+str(22-5+i-(137-5+2))

    for i in range(182-3+2, 204+2+2): #TM4
        OPRM_BW_dic[i]='TM4.'+str(40-3+i-(182-3+2))

    for i in range(229-5+2, 268+2): #TM5
        OPRM_BW_dic[i]='TM5.'+str(35-5+i-(229-5+2))

    for i in range(275-7+2, 305+2+2): #TM6
        OPRM_BW_dic[i]='TM6.'+str(30-7+i-(275-7+2))

    for i in range(316-5+2, 351+2+10): #TM7
