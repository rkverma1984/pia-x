...
metadata_path = current_dir + '/unit_set/inputs/trajectories/'

outdir = current_dir + '/unit_set/gpcr_mdtraj/'  # dir to save data for each of the frame.
outdir_av_and_diff_mat = current_dir + '/unit_set/traj_mdtraj/'  # dir to save average matrices for each of the state as well as the differential
# matrices.
outdir_pymol = current_dir + '/unit_set/gpcr_pymol/'  # dir to save data for each of the frame.
outdir_av_and_diff_mat_pymol = current_dir + '/unit_set/traj_pymol/'  # dir to save average matrices for each of the state as well as the
# differential matrices.
plot_dir = current_dir + '/unit_set/plots/'

suffix = '_analysis/traj/'  # suffix for the dir, where traj and the pdb are saved
stride = 2  # skip traj every 2 frames
mass = 1
protein_name = 'd3r'
trajectory_suffix = 'protein*.dcd'  # change *protein*.dcd if using dcd files
topology_suffix = 'cms.wrap.protein.pdb'  # suffix for topology file. topology file must be in the same dir as dcd file.

cluster_data_file = 'unit_set/inputs/cluster_unit_test.dat'  # name of the file containing PIA derived cluster information for frames from a given
#  trajectory.
...
sigma_cutoff = 0  # cutoff used to select helical vectors. default value in pymol script is 1.5
