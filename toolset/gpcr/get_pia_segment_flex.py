...
def get_segments_distance(u, segmanet_list1, segmanet_chainid1, labels1, segmanet_list2, segmanet_chainid2, labels2):
    ...
    for i in range(len(segmanet_list1)):
        ...
        # _selseg1 = u.select_atoms( 'protein and segid PRO' + segmanet_chainid1 + ' and resid ' + segmanet_list1[i] + ' and (not type H) and backbone')
    ...
    for i in range(len(segmanet_list2)):
        ...
        # _selseg1 = u.select_atoms('protein and segid PRO' + segmanet_chainid2 + ' and resid ' + segmanet_list2[i] + ' and (not type H) and backbone')
    ...
    for i in range(len(segmanet_list1)):
        ...
        for j in range(len(segmanet_list2)):
            ...
            for k in range(_len_va):
                _ls_angle.append(np.linalg.norm(_va[k]- _vb[k]))

            colnames.append("seg_dist_"+labels1[i] + "_" + labels2[j])
            all_seg_dist.append(_ls_angle)
...
if __name__ == '__main__':
    ...
    # _df.to_hdf(output_dir + '/df_seg_dist_rec_ga_flex.hdf', key='df', mode='w')

    # segments distance: rec vs rec (PIA)
    _df=get_segments_distance(u, rec_segments_pia, rec_chainid, rec_labels_pia, rec_segments_pia, rec_chainid, rec_labels_pia)
    _df.to_hdf(output_dir + '/df_seg_dist_rec_rec_flex.hdf', key='df', mode='w')
