...
def _calc_interface(t):
    ...
    '''
    cutoff=8
    sel_AR = atomsel('(protein and (chain A) and (within '+str(cutoff)+' of chain R)) or (protein and (chain R) and (within '+str(cutoff)+' of chain A))')
    sel_A = atomsel('protein and (chain A) and (within '+str(cutoff)+' of chain R)')
    sel_R = atomsel('protein and (chain R) and (within '+str(cutoff)+' of chain A)')
    sel_AR.frame = t
    sel_AR.update()
    sel_A.frame=t
    sel_A.update()
...
def get_interface(pdbin, dcdin, Verbose=False):
    ...
    if (False):
        ...
        sel_AR = atomsel('protein and chain A R')
        sel_A = atomsel('protein and chain A')
        sel_R = atomsel('protein and chain R')
        # sasa_AR, points = sel_AR.sasa(srad=1.4, points=True)
        # sasa_A, points = sel_A.sasa(srad=1.4, points=True)
        # sasa_R, points = sel_R.sasa(srad=1.4, points=True)
...
if __name__ == '__main__':
    ...
    with open(output_dir+'/gpcr_interface_RA.p', 'wb') as fp:
