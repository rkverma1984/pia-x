...
class Frequency:
    ...
    def contact_freq_vmd(self):
        ...
        # print ('lig chain ID', self._ligand_chain_ID)
        # print ('rec chain ID', self._receptor_chain_ID)
        ...
        for r in tqdm( range( len( rec_resids ) ) ):  # control by residue index
            ...
            for l in range( len( lig_resids ) ):
                ...
                # receptor = atomsel("resid "+str(r_id)+" and not hydrogen and chain "+self._receptor_chain_ID)
                # ligand = atomsel("resid "+str(l_id)+" and not hydrogen and chain "+self._ligand_chain_ID)
                if not self._bootstrap:
                    ...
                    for t in range( 1, num_steps ):
                        ...
                        if receptor.contacts( ligand, self._cutoff )[0] != []:
                            count += 1
                            # print (r_id)
                    freq = float( count ) / float( num_steps )
                else:
                    freq = 0
        ...
        print('sorted not_empty_yindex', not_empty_yindex)
        print('')
        print('ligand_labels, not sorted', ligand_labels)
        print('')
        ...
        receptor_labels = sorted( list( set( receptor_labels ) ) )
        #ligand_labels = sorted( list( set( ligand_labels ) ) )
        ligand_labels = sorted( list( set( ligand_labels ) ), key = lambda x: (len (x), x) )
        print('sorted ligand_labels', ligand_labels)
        print('')
        print('')
        return filtered_frequency_matrix, receptor_labels, ligand_labels
        
    #timeit.timeit(contact_freq_vmd)
                    ...
                    for t in range( 1, num_steps ):
                        ...
                        if dist < self._cutoff:
                            ...
                            receptor_labels.append(
                                "%3d" % (r_id) + ' ' + _resid2resname( r_id, self._receptor_chain_ID )[0] )
                            ligand_labels.append(
                                "%3d" % (l_id) + ' ' + _resid2resname( l_id, self._ligand_chain_ID )[0] )
                            not_empty_xindex.append( r )
                            not_empty_yindex.append( l )
            ...
            for r in tqdm( range( len( rec_resids ) ) ):  # control by residue index
                ...
                for l in range( len( lig_resids ) ):
                    ...
                    if dist < self._cutoff:
                        ...
                        receptor_labels.append(
                            "%3d" % (r_id) + ' ' + _resid2resname( r_id, self._receptor_chain_ID )[0] )
                        ligand_labels.append( "%3d" % (l_id) + ' ' + _resid2resname( l_id, self._ligand_chain_ID )[0] )
                        not_empty_xindex.append( r )
                        not_empty_yindex.append( l )
