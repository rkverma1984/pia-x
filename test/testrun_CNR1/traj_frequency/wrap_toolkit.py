...
def merge_AB_metrics(new_matrix,B_new_matrix,filtered_rec_labels,B_filtered_rec_labels,filtered_lig_labels,B_filtered_lig_labels,cutoff):
    ...
    merged_matrix = np.full((len(union_labels_keys),len(filtered_lig_labels+B_filtered_lig_labels)), cutoff, dtype= float)
    filter_y_indexes_Ga = []
    filter_y_indexes_Gb = []
...
def make_union_list(label_list1, label_list2):
    ...
    print('Function: make_union_list')
    print('')
    ...
    for u in union_list:
        if not 'Gb' in u:
            ...
            turpled_union_list.append(("%3d" % int(u.split()[0]), u.split()[1]))
        else:
    ...
    print(sorted_union_list, 'sorted')
    print('')
    print('')
...
def expanded_matrix(matrix, union_xlables, union_ylables, xlabels, ylabels):
    ...
    print ('before main loops')
    print("ylabels read by expanded_matrix function")
    print(ylabels)
    print('')
    ...
    for i in range(len(union_xlables)):
        if not union_xlables[i] in xlabels:
            print(i, union_xlables[i])
            need_added_fen_x_indexes.append(i)
        else:
            existed_fen_xindexes.append(i)

    print('x - receptor')
    print('not existed', need_added_fen_x_indexes)
    print('existed', existed_fen_xindexes)
    print('')    
    ...
    print ("PROBLEM is the format mismatch between ylabels and union_ylables") 
    print ('before ylabel loop')
    print ('ylables', ylabels)
    print ('union_ylables', union_ylables)
    print ('')
    for j in range(len(union_ylables)):
        if not union_ylables[j] in ylabels:
            print(j, union_ylables[j])
            need_added_fen_y_indexes.append(j)
        else:
            existed_fen_yindexes.append(j) 

    
    print('y - Galpha')
    print('not existed', need_added_fen_y_indexes)
    print('existed', existed_fen_yindexes)    
    #print ('PROBLEM')   
    ...
    for i in range(len(existed_fen_xindexes)):
        for j in range(len(existed_fen_yindexes)):
            ...
            #print(existed_fen_xindexes[0],existed_fen_yindexes[0],matrix[0][0])
...
def filter_empty_matrix(matrix, xlabel, ylabel):
    ...
    empty_x = sorted(list(set(empty_x)))
    empty_y = sorted(list(set(empty_y)))
...
def filter_duplicate_matrix(matrix,xlabel, ylabel):
    ...
    for n in non_repeatable_ylabel:
        if not 'Gb' in n:
            ...
            tem_y.append(("%3d" % int(n.split()[0]), n.split()[1]))
        else:
    ...
    print('')
    print('filter_duplicate function')
    print(sorted_ylabel)
...
def reorder_matrix(matrix, xlabel, ylabel):
    ...
    for y in range(len(ylabel)):
        if 'Gb' in  ylabel[y]:
            reorder_ylabel_index.append(y)
    print(len(reorder_ylabel_index))
    print(len(reorder_xlabel_index))
